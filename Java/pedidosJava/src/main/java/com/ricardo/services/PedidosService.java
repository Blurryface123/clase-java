...
public class PedidosService {
	...
	public Response getPedido(@PathParam("id") int pid) {
		...
		PedidosManager pm = PedidosManager.getInstance();
		List<Pedido> pedidosrec = pm.getPedidos();
		for (Pedido pedido : pedidosrec) {
...
//		if (pedidoRet == null) {
	...
	public Response deletePedido(@PathParam("id") int pid) {
		...
		if(pid>0) {
			try {
				return Response.status(200).entity(PedidosManager.getInstance().deletePedidos(pid)).build();
			}catch(Exception e){
				System.out.println("Exception"+e.getMessage());
				return Response.status(500).entity(PedidosManager.getInstance().deletePedidos(pid)).build();
			}
